...
main:
  ...
  add $t0, $0, $0						# loopCounter  = 0;
  
  #Reserving registers: 
  #		Red - $t1, Green - $t2, Blue - $t3, 
  #		Currennt Pixel - $t4, Grey (Answer) - $t5

loadArray:
	lw		$t4, pixels($t0)			#Getting the values from the pixel array
	beq		$t4, -1, exit				#If -1 is reached, terminate the program
	
	#Processing the read pixels
	andi	$t3, $t4, 0xff					#Blue
	srl		$t4, $t4, 8				#Bit logical shift right, by 8 ( >> 8)
	andi	$t2, $t4, 0xff					#Green
	srl		$t4, $t4, 8				#Bit logical shift right, by another 8 ( >> 8)
	andi	$t1, $t4, 0xff					#Red
	
	#Getting the gray pizel value
	jal		rgb_to_gray				# $t5 = gray
	
	#Printing out the gray pixel value
	addi	$v0, $0, 1					#System call 1, printing an Integer
	add	$a0, $0, $t5					#Put the gray value into register $a0
	syscall							#Print the Integer
	
	addi	$v0, $0, 4					#System call 4, printing string
	la	$a0, newline					#Loading the address of newline
	syscall
	
	#Increment loopCounter
	addi	$t0, $t0, 4					#loopCounter += 1;
	#Loop back
	j		loadArray
...
rgb_to_gray:            					# procedure to calculate gray = (red + green + blue) / 3
	...
	add		$t9, $t1, $t2				#Add read and green to the total
	add		$t9, $t9, $t3				#Add blue to the runing total
	div		$t5, $t9, 3				#Integer division by 3
	
	jr		$ra					#Return to the stored address, from link
#------- INSERT YOUR CODE HERE -------
#
